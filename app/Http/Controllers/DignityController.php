...
use View;
use App;
use App\Blocks\Dignity\Tabs;
use App\Blocks\Dignity\Grid as GridReview;
...
class DignityController extends Controller
{
    function __construct()
    {
        ...
        $this->setGridId('dignities');
        $this->setTabsId('dignities');
        $this->setResource('App\Models\Dignity');
        $this->setSingularKey('dignities');
        $this->setPluralKey('dignities');
        $this->setModelClass('App\Models\Dignity');
    }
    ...
    public function indexWithoutParam()
    {
        ...
        $product = [];
        View::share([
            'pageTitle' => 'Phẩm Chất Cá Nhân',
            'product' => $product,
        ]);
        return view('dignity.edit')->with('reviewGrid', $reviewGrid);
    ...
    {   $params = $this->_parseFilter($filter);
        $gridReview = new GridReview('dignities', 'App\Models\Dignity', 'dignities', $params);
    ...
    public function massDelete()
    {
        ...
        if (isset($data['errors)'])) {
            return Redirect::route('dignities.list')->with('error', $data['errors'][0]['message'])->withInput();
        } else {
            $count = count($data);
            return Redirect::route('dignities.list')->with('success',
    ...
    {
        $this->setPageTitle("Phẩm Chất Cá Nhân");
    ...
    public function reviews($productId = null)
    {
        ...
        $param['filter']= $input;
        $gridReview = new GridReview('dignities', 'App\Models\Dignity', 'dignities', $param);
        $this->setGrid($gridReview);
//            $this->setPageTitle(Lang::get('catalog.manage_products'));
    ...
    {
        $data = App\Models\Dignity::find($id);
        ...
        $this->setPageTitle(Lang::get('dignities.edit_CVCM') . ' # ' . $data['_id']);
        $tabs = new Tabs($this->getTabsId());
        $this->setTabs($tabs);
        return $this->loadTabs();
...
//        App\Helpers\AutoTest::PhamChatCaNhan($input['level_id'],$input['room_id'],$input['chucdanh_id']);
//        return Redirect::route('dignities')
//            ->with('success', Lang::get('messages.the_role_has_been_saved'));
        ...
        if (isset($data['errors']))
            return $id ?
                Redirect::route('dignities.edit', [$id])
                    ->with('error', $data['errors'][0]['message'])
                    ->withInput() :

                Redirect::route('dignities.create')
                    ->with('error', $data['errors'][0]['message'])
                    ->withInput();
        elseif (Input::get('tab')) {
            $id = $data['_id'];
            return Redirect::route('dignities.edit', [$id, 'tab' => Input::get('tab')])->with('success',
