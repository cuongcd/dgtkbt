...
use Input;
use App\Blocks\Point\Grid;
use Redirect;
use Lang;
use App;
use App\Blocks\Point\Tabs;
...
class PointController extends Controller
{
    function __construct()
    {
        ...
        $this->setGridId('points');
        $this->setTabsId('points');
        $this->setResource('App\Models\Point');
        $this->setSingularKey('points');
        $this->setPluralKey('points');
        $this->setModelClass('App\Models\Point');
    }
    ...
    public function indexWithoutParam()
    {
        ...
        $this->setGrid($grid);
        $this->setPageTitle('Danh Sách Loại Xếp Loại Lao Động');
    ...
    public function massDelete()
    {
        ...
        if (isset($data['errors)'])) {
            return Redirect::route('points.list')->with('error', $data['errors'][0]['message'])->withInput();
        } else {
            $count = count($data);
            return Redirect::route('points.list')->with('success',
    ...
    {
        $this->setPageTitle('Thêm Loại Xếp Loại');
    ...
    {
        $data = App\Models\Point::find($id);

        if (isset($data['errors'])) {
            return Redirect::route('points.list')
                ->with('error', $data['errors'][0]['message'])
                ->withInput();
        }
        App::instance($this->getTabsId(), $data);
        $this->setPageTitle('Loại ' . ' # ' . $data['name']);
        $tabs = new Tabs($this->getTabsId());
        $this->setTabs($tabs);
        return $this->loadTabs();
    }
...
//            App\Models\Role::find($data['_id'])->permission()->sync($category_id);
        ...
        if (isset($data['errors']))
            return $id ?
                Redirect::route('points.edit', [$id])
                    ->with('error', $data['errors'][0]['message'])
                    ->withInput() :

                Redirect::route('points.create')
                    ->with('error', $data['errors'][0]['message'])
                    ->withInput();
        elseif (Input::get('tab')) {
            $id = $data['_id'];
            return Redirect::route('points.edit', [$id, 'tab' => Input::get('tab')])->with('success',
